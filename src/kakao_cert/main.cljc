(ns kakao_cert.main
  "Kotodama WASM entrypoint for the Kakao_cert clean-room actor (L4) — Clojure port.
  ...
  facts (`kakao_cert.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kakao_cert")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"kakao_cert.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kakao_cert-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kakao_cert-compat/ready)
